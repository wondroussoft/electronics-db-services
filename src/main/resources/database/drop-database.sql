...
-- ERROR:  database "electronics" is being accessed by other users
-- DETAIL:  There are 12 other sessions using the database.

UPDATE pg_catalog.pg_database SET datallowconn=false WHERE datname='electronics';
--SELECT pg_catalog.pg_terminate_backend(procpid) FROM pg_catalog.pg_stat_activity WHERE datname='electronics';
